...
export default async function Home() {
  ...
  const renderedScores = data.map((item: CategoryItem) => {
    return (
      <div
        className={`flex flex-row items-center justify-between my-4 p-4 sm:my-[1rem] sm:p-[0.875rem] rounded-xl bg-primary${item.color}/5`}
        key={item.category}
      >
        <div className="flex flex-row items-center">
          <div className="relative h-4 w-4 sm:w-5 sm:h-5 mr-3 ml-[0.15rem]">
            <Image className="" src={item.icon} fill alt="" />
          </div>
        ...
        <div>
          <span className="sm:text-lg font-bold">{item.score} </span>{" "}
          <span className="sm:mx-[0.2rem] sm:text-lg font-thin text-neutralGray/50">
            {" "}
            / 100
          </span>
        </div>
